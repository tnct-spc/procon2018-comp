...
public:
   ...
   // bool isPlaced(const unsigned int x, const unsigned int y);
    ...
    std::vector<double> calcSituationFeature(std::vector<std::pair<std::tuple<int, int, int>, std::tuple<int, int, int>>> agent_pos,int side_of);
    //まず全agentの行動のposを投げて,第二引数にどっち視点で見るか(下の行に書いてある敵味方の話)
    //1 -> 領域点の差の変化 2 -> マスの得点の差の変化 3 -> 味方agents同士の距離の差 4 -> 敵agent同士の距離の差 5 -> 味方と敵の距離の総和の差 6 -> 味方agentと中央との距離の総和の差(大きければ大きいほど外側) 7 -> 敵のagentと中心の距離の総和の差
private:
