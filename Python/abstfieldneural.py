...
def abst_field_neural():
    ...
    out_data = np.empty((len(data), 1), dtype=np.float32)

    for index in range(len(data)):
        for i in range(30):
            inp_data[index][i] = data[index][i]
        out_data[index][0] = 0 if data[index][30] == data[index][31] else 1 if data[index][30] > data[index][31] else 2
    ...
    network.calc_neural([600, 500, 1], train_data, test_data, const.result_path + '_abst_field/', '_abst_field', int(min(const.max_train, train_size)), int(min(const.max_test, int(const.test_data_per * len(data)))), const.epoch)
