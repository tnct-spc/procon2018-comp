...
def raw_field_neural():
    ...
    network.calc_neural([500, 1], train_data, test_data, const.result_path + '_raw_field/', '_raw_field', int(min(const.max_train, train_size)), int(min(const.max_test, int(const.test_data_per * len(bin_data)))), const.epoch)
