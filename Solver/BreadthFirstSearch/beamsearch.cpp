       ...
       // cout<<"turn開始"<<endl;
        if(turn==0){
           ...
           //         cout<<"ｂのfor文"<<endl;
             ...
             //       cout<<"contineで生き残ったもの"<<endl;
                    int state1 = field.getState(agent1.first+age1.at(a).first,agent1.second+age1.at(a).second).first;
                    int state2 = field.getState(agent2.first+age2.at(b).first,agent2.second+age2.at(b).second).first;
         ...
         //   cout<<"2回め以上"<<endl;
            ...
            for(int range = 0;range < std::min(beam_range,ins_size);range++){
                std::pair<int,std::tuple<procon::Field,std::tuple<int,int,int>,std::tuple<int,int,int>>> ins_value = beam.front();
                ...
                for(int a = 0;a < 8;a++){
                 //   cout<<"ne"<<endl;
                    ...
                    for(int b = 0;b < 8;b++){
                        ...
                        int Value = Evaluation_Field(ins_field);
                        ins_beam.push_back(std::make_pair(Value,std::make_tuple(ins_field,std::get<1>(ins_value.second),std::get<2>(ins_value.second))));
                        cout<<count<<endl;
                        count++;
                    }
                }
