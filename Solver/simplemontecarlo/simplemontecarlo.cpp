...
const std::pair<std::tuple<int,int,int>, std::tuple<int,int,int>> SimpleMonteCalro::agentAct(int side){
    ...
    std::tuple<int,int,int> agent_1_tuple = std::make_tuple( (field.getState(agent_1_pos.first, agent_1_pos.second) == (side == 0? 2 : 1 ) ? 2 : 1 ),
                                                             agent_1_pos.first, agent_1_pos.second);

    std::tuple<int,int,int> agent_2_tuple = std::make_tuple( (field.getState(agent_2_pos.first, agent_2_pos.second) == (side == 0? 2 : 1 ) ? 2 : 1 ),
                                                             agent_2_pos.first, agent_2_pos.second);
