#include "useabstmontecarlo.h"

UseAbstMonteCarlo::UseAbstMonteCarlo(const procon::Field& field, int final_turn, bool side) :
...
const std::pair<std::tuple<int,int,int>, std::tuple<int,int,int>> UseAbstMonteCarlo::agentAct(int now_turn){
    ...
    auto playout = [&](int move, int cpu){
        ...
        std::vector<double> val_1 = values.at(retRandom(0,values_size - 1));
        std::vector<double> val_2 = values.at(retRandom(0,values_size - 1));

        GeneticAgent* side_1 = new GeneticAgent(10, false);//ここはアルゴリズムによってサイズが変わる
        GeneticAgent* side_2 = new GeneticAgent(10, false);
        ...
        UseAbstractData algo(field, final_turn, side, *side_2);
...
void UseAbstMonteCarlo::setParams(std::vector<std::pair<QString, double>> params)
{

}

void UseAbstMonteCarlo::setRandomParams(std::vector<std::pair<QString, double>> params,bool flag){
